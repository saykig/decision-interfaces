"""Fresh R19B mathematical-interface compilation and transitive axiom audit."""
import argparse
import hashlib
import json
import os
...
MODULES = ["Interface", "Realization", "Attainment", "RationalQueries", "Reuse", "TreeBudget"]
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="di-r19b-proof-") as temp:
        build = Path(temp)
        env = dict(os.environ, LEAN_PATH=os.pathsep.join([str(build)] + paths))
        for module in MODULES:
            source = ROOT / (module + ".lean")
            content = source.read_text()
            if re.search(r"\b(sorry|admit|axiom)\b", re.sub(r"/-.*?-/", "", content, flags=re.S)):
                raise SystemExit("Unproved declaration token in " + source.name)
            names = re.findall(r"^(?:def|abbrev|lemma|theorem|structure|inductive)\s+([\w.]+)", content, re.M)
            if not names:
                raise SystemExit("No declarations in " + module)
            declarations.extend("DecisionInterface." + name for name in names)
